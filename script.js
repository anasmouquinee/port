...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "import pandas as pd\nfrom minio import Minio\nfrom datetime import datetime\n\ndef upload_to_data_lake(df: pd.DataFrame, bucket_name: str, object_prefix: str):\n    \"\"\"Uploads a DataFrame to MinIO as a Parquet file.\"\"\"\n    client = Minio(\"minio:9000\", access_key=\"minio_admin\", secret_key=\"anaskaelar\", secure=False)\n    timestamp = datetime.now().strftime(\"%Y%m%d%H%M%S\")\n    object_name = f\"{object_prefix}/data_{timestamp}.parquet\"\n    \n    with pd.io.common.BytesIO() as buffer:\n        df.to_parquet(buffer, index=False)\n        buffer.seek(0)\n        client.put_object(bucket_name, object_name, buffer, len(buffer.getvalue()))\n        print(f\"Uploaded {object_name} to MinIO bucket {bucket_name}\")",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Kafka",
    ...
    "desc": "Developed a comprehensive enterprise lakehouse platform for real-time media intelligence, leveraging a distributed architecture for news article ingestion, processing, and visualization. This system integrates advanced data engineering patterns, including Medallion Architecture and container orchestration, to deliver scalable and insightful analytics.",
    "features": [
      "Implemented a distributed lakehouse for real-time media intelligence, handling ingestion, processing, and visualization of news data.",
      "Orchestrated complex data pipelines with Apache Airflow, leveraging a Medallion Architecture for data quality and transformation.",
      "Utilized Apache Kafka for high-throughput, real-time event streaming and MinIO as a scalable object storage data lake.",
      "Deployed a containerized infrastructure using Docker Compose and prepared for Kubernetes, ensuring scalability and robust service integration."
    ],
  ...
  },
  {
    ...
    "codeSnippet": "import axios from 'axios';\n\nexport async function processAIPrompt(promptText: string): Promise<string> {\n  try {\n    const response = await axios.post('/api/ai-inference', { prompt: promptText });\n    return response.data.generatedText; // Assuming a structured response\n  } catch (error) {\n    console.error(\"Error processing AI prompt:\", error);\n    return \"Failed to get AI response.\";\n  }\n}",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "OmniPulse AI Studio is a robust web application designed for interactive AI development and testing, providing a streamlined environment for experimenting with AI prompts and APIs. Built with React, TypeScript, and Vite, it leverages modern frontend tooling and a modular backend architecture to ensure high performance and developer efficiency.",
    "features": [
      "Interactive AI Prompting & API Integration",
      "High-Performance Frontend Architecture with React, TypeScript, and Vite",
      "Automated Code Quality and Linting using Oxlint",
      "Containerized Deployment Strategy via Docker"
    ],
