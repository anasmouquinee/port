...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "import json\nfrom datetime import datetime\n\ndef transform_article_to_silver_layer(raw_article_data: dict) -> dict:\n    article_id = raw_article_data.get(\"id\", \"unknown\")\n    title = raw_article_data.get(\"title\", \"\").strip()\n    source_name = raw_article_data.get(\"source\", \"N/A\")\n    clean_title = \" \".join(title.split())\n    return {\n        \"article_uuid\": article_id,\n        \"standardized_title\": clean_title,\n        \"source\": source_name,\n        \"processing_ts\": datetime.utcnow().isoformat()\n    }",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Airflow",
    ...
    "desc": "This comprehensive, distributed enterprise lakehouse platform enables real-time media intelligence through end-to-end data ingestion, processing, and visualization of news articles. It leverages a medallion architecture to ensure robust data management and analytics from raw sources to curated insights.",
    "features": [
      "Distributed data ingestion and real-time stream processing with Apache Kafka.",
      "Scalable Enterprise Lakehouse architecture (MinIO & PostgreSQL) adhering to the Medallion pattern.",
      "Automated ETL/ELT pipelines orchestrated by Apache Airflow for robust data transformation.",
      "Comprehensive media intelligence visualization and monitoring via Apache Superset, Prometheus, and Grafana."
    ],
  ...
  },
  {
    ...
    "codeSnippet": "async function generateAIResponse(prompt: string, model: string = \"omni-v1\"): Promise<string> {\n  const payload = { prompt, model };\n  const response = await fetch('/api/inference', {\n    method: 'POST',\n    headers: { 'Content-Type': 'application/json' },\n    body: JSON.stringify(payload),\n  });\n\n  if (!response.ok) {\n    throw new Error(`API error: ${response.statusText}`);\n  }\n  const { output } = await response.json();\n  return output as string;\n}",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "OmniPulse AI Studio provides a robust web-based environment for developing and iterating on AI prompts and applications. It leverages a modern React and TypeScript front-end integrated with a powerful API to facilitate rapid AI solution prototyping and deployment.",
    "features": [
      "High-performance frontend architecture with Vite, React, and TypeScript",
      "Integrated AI prompting and testing tools for rapid iteration and evaluation",
      "Scalable API layer designed for complex AI service integrations and data handling",
      "Containerized development and deployment workflows using Docker for consistency"
    ],
