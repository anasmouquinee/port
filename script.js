...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "import pandas as pd\n\ndef clean_news_data(raw_df: pd.DataFrame) -> pd.DataFrame:\n    processed_df = raw_df.copy()\n    processed_df['title_clean'] = processed_df['title'].str.strip().str.upper()\n    processed_df['source_std'] = processed_df['source'].str.replace(' ', '_').str.upper()\n    processed_df['pub_year'] = pd.to_datetime(processed_df['published_at']).dt.year\n    processed_df['text_len'] = processed_df['content'].apply(lambda x: len(x) if x else 0)\n    return processed_df",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Kafka",
    ...
    "desc": "Developed a comprehensive, distributed platform for real-time media intelligence, designed to ingest, process, and visualize news article data. It leverages an enterprise lakehouse architecture, encompassing automated scraping, ETL/ELT pipelines, and advanced analytics for dynamic media insights.",
    "features": [
      "Real-time Media Data Ingestion & Scraping from diverse news sources",
      "Distributed Enterprise Lakehouse Architecture with Medallion layers (Bronze, Silver, Gold)",
      "Automated ETL/ELT Pipelines orchestrated via Apache Airflow for data transformation and quality",
      "Comprehensive Real-time Monitoring & Business Intelligence Dashboards using Grafana and Superset"
    ],
  ...
  },
  {
    ...
    "codeSnippet": "interface AISettings { temperature: number; maxTokens: number; }\n\n/** Simulates interaction with an external AI model. */\nexport async function processPromptWithAI(\n  userPrompt: string,\n  settings: AISettings\n): Promise<string> {\n  // In a real application, this would call an external AI API (e.g., OpenAI)\n  console.log(`Processing prompt: \"${userPrompt}\" with settings:`, settings);\n  const simulatedResponse = `AI response to \"${userPrompt}\" at temp ${settings.temperature}.`;\n  await new Promise(resolve => setTimeout(resolve, 500)); // Simulate async work\n  return simulatedResponse;\n}",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "OmniPulse AI Studio is a sophisticated full-stack application providing an interactive environment for designing, testing, and refining AI prompts and models. It leverages a modern frontend built with React, TypeScript, and Vite, coupled with a robust backend architecture for efficient AI model interaction and data management.",
    "features": [
      "Interactive AI Prompt & Model Interaction for rapid development",
      "High-Performance Type-Safe Frontend with React, TypeScript, and Vite",
      "Containerized & Scalable Backend Architecture using Docker for robust AI services",
      "Automated Code Quality and Hot-Reloading for efficient development workflow"
    ],
