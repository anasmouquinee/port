...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "import pandas as pd\n\ndef transform_news_data(df: pd.DataFrame) -> pd.DataFrame:\n    \"\"\"Applies cleaning and enrichment for Silver layer.\"\"\"\n    df['title_clean'] = df['title'].str.lower().str.strip()\n    df['source_domain'] = df['url'].apply(lambda x: pd.NA if not x else x.split('/')[2])\n    df['publish_dt'] = pd.to_datetime(df['published_at'], errors='coerce')\n    # Entity extraction placeholder\n    df['keywords'] = df['text_content'].apply(\n        lambda x: ['AI', 'ML'] if 'AI' in str(x) else []\n    )\n    df.drop(columns=['raw_data', 'url'], errors='ignore', inplace=True)\n    return df",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Kafka",
    ...
    "desc": "Global NewsStream is a comprehensive, distributed enterprise platform designed for real-time media intelligence, addressing the challenge of ingesting, processing, and visualizing vast volumes of news article data. It leverages a modern lakehouse architecture with robust ETL/ELT pipelines, enabling scalable data scraping, storage, and advanced analytics for media insights.",
    "features": [
      "Implemented a scalable, distributed lakehouse architecture for real-time ingestion, processing, and analysis of media data.",
      "Orchestrated complex ETL/ELT pipelines with Apache Airflow, adhering to a Medallion Architecture for data quality and governance.",
      "Leveraged Apache Kafka for real-time stream processing and MinIO as a robust, S3-compatible data lake.",
      "Deployed a fully containerized infrastructure using Docker and Kubernetes, ensuring high availability and seamless scalability."
    ],
  ...
  },
  {
    ...
    "desc": "OmniPulse AI Studio is a robust web application built on React and Vite, designed to provide a comprehensive platform for AI development and interaction. It leverages a modern frontend stack with server-side API integration, showcasing a scalable architecture for AI-driven services.",
    "features": [
      "Modular React/TypeScript frontend with Vite for optimized performance",
      "Robust API layer for seamless interaction with AI services",
      "Automated code quality and type safety enforced via Oxlint and TypeScript",
