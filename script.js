...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "from airflow import DAG\nfrom airflow.operators.bash import BashOperator\nfrom datetime import datetime\n\nwith DAG(\n    dag_id='news_lakehouse_etl',\n    start_date=datetime(2023, 1, 1),\n    schedule_interval='@daily',\n    catchup=False,\n) as dag:\n    scrape_news = BashOperator(task_id='scrape_data', bash_command='python /app/scrapers/main.py')\n    load_bronze = BashOperator(task_id='load_bronze_layer', bash_command='python /app/pipelines/load_minio.py')\n    process_silver = BashOperator(task_id='process_silver_layer', bash_command='python /app/pipelines/transform.py')\n    load_gold = BashOperator(task_id='load_gold_layer', bash_command='python /app/pipelines/aggregate.py')\n\n    scrape_news >> load_bronze >> process_silver >> load_gold",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Kafka",
    ...
    "desc": "Developed a comprehensive, distributed enterprise lakehouse platform for real-time ingestion, processing, and visualization of news articles, providing critical media intelligence. Engineered a robust data pipeline utilizing a medallion architecture, ETL/ELT processes, and containerized services to manage and analyze high-volume, dynamic data streams.",
    "features": [
      "End-to-end distributed platform for real-time news data ingestion, processing, and visualization.",
      "Implemented a robust Medallion Architecture (Bronze, Silver, Gold layers) within a Data Lakehouse (MinIO, PostgreSQL).",
      "Automated ETL/ELT pipelines orchestrated by Apache Airflow ensuring data quality and transformations.",
      "Scalable deployment leveraging Docker and Kubernetes, with comprehensive monitoring via Prometheus and Grafana."
    ],
  ...
  },
  {
    ...
    "codeSnippet": "import React, { FC } from 'react';\n\ninterface AIStatusProps {\n  status: 'active' | 'inactive' | 'processing';\n  lastUpdate: Date;\n}\n\nconst AIStatusDisplay: FC<AIStatusProps> = ({ status, lastUpdate }) => (\n  <div className={`status-display status-${status}`}>\n    <h3>AI Engine Status: {status.toUpperCase()}</h3>\n    <p>Last Updated: {lastUpdate.toLocaleString()}</p>\n  </div>\n);",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "Developed a modern, high-performance web application, OmniPulse AI Studio, serving as an intuitive user interface for AI-driven workflows and data visualization. Leveraging React, TypeScript, and Vite, this project prioritizes a streamlined development experience, robust type safety, and efficient bundle management.",
    "features": [
      "Optimized Frontend Architecture: Utilizes React with Vite for a high-performance, HMR-enabled development and production environment.",
      "Robust Type Safety: Implements TypeScript across the codebase to ensure strong type checking and enhanced maintainability.",
      "Advanced Code Quality Enforcement: Integrates Oxlint, configured for type-aware linting, to enforce strict code standards and identify potential issues early.",
      "Containerized Deployment Workflow: Leverages Docker for consistent environment provisioning and streamlined deployment readiness."
    ],
