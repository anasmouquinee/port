...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "from airflow.operators.python import PythonOperator\n\ndef transform_raw_to_silver(source_path: str, dest_path: str):\n    \"\"\"\n    Reads raw data from object storage, applies transformations,\n    and writes to a silver layer.\n    \"\"\"\n    print(f\"Reading raw data from: {source_path}\")\n    # Placeholder for actual data processing logic (e.g., Pandas)\n    transformed_data = f\"Transformed content from {source_path}\"\n    print(f\"Writing transformed data to: {dest_path}\")\n\n# Example Airflow task definition\ntransform_task = PythonOperator(\n    task_id='transform_articles_to_silver',\n    python_callable=transform_raw_to_silver,\n    op_kwargs={\n        'source_path': 's3://raw-zone/news/{{ ds }}.json',\n        'dest_path': 's3://silver-zone/news/{{ ds }}.parquet'\n    },\n)",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Kafka",
    ...
    "desc": "Engineered a distributed enterprise lakehouse platform for real-time media intelligence, capable of ingesting, processing, and visualizing news articles from diverse sources. This solution implements a robust medallion architecture with automated ETL/ELT pipelines, providing dynamic insights into global media trends.",
    "features": [
      "Distributed Real-Time Data Ingestion with Kafka for diverse news sources",
      "Scalable Enterprise Lakehouse Architecture (Medallion Pattern) with MinIO and PostgreSQL",
      "Automated ETL/ELT Workflows Orchestrated by Apache Airflow",
      "Interactive Media Intelligence Dashboards via Apache Superset for Trend Analysis"
    ],
  ...
  },
  {
    ...
    "codeSnippet": "import axios from 'axios';\n\ninterface AIPromptResponse {\n  output: string;\n  tokens: number;\n}\n\nexport async function submitAIPrompt(prompt: string): Promise<AIPromptResponse> {\n  try {\n    const response = await axios.post<AIPromptResponse>('/api/generate', { prompt });\n    return response.data;\n  } catch (error) {\n    console.error(\"AI prompt submission failed:\", error);\n    throw new Error(\"Failed to process AI prompt.\");\n  }\n}",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "Developed a robust, modern AI studio platform for full-stack interaction with artificial intelligence models. This project offers a high-performance web interface built with React and TypeScript, enabling efficient development and deployment of AI-powered features.",
    "features": [
      "Developed a high-performance, type-safe frontend with React and TypeScript, optimized by Vite",
      "Implemented robust API integration for seamless interaction with AI backend services",
      "Ensured consistent, portable deployment through Docker containerization",
      "Integrated real-time data streaming capabilities using Apache Kafka for event-driven architectures"
    ],
