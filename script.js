...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "codeSnippet": "export async function getTestAIResponse(testPrompt: string): Promise<string> {\n  const payload = { testInput: testPrompt, model: 'development-v1' };\n  const response = await fetch('/api/test-ai-endpoint', {\n    method: 'POST',\n    headers: { 'Content-Type': 'application/json' },\n    body: JSON.stringify(payload),\n  });\n\n  if (!response.ok) throw new Error(`AI test failed: ${response.status}`);\n  const data = await response.json();\n  return data.testOutput; // Expected output key\n}",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "This project develops a cutting-edge AI studio interface leveraging React and Vite for a seamless user experience. It provides a robust platform for interacting with and managing AI models, emphasizing performance and code quality through modern web standards.",
    "features": [
      "High-performance web frontend powered by React and Vite",
      "Containerized application deployment using Docker for scalability",
      "Enhanced code quality and maintainability with type-aware Oxlint rules",
      "Modular API architecture enabling flexible AI model interaction"
    ],
