...
const EMBEDDED_PROJECTS = [
  ...
  },
  {
    ...
    "hasCodeSnippet": true,
    "codeLanguage": "python",
    "codeSnippet": "import requests\nfrom kafka import KafkaProducer\nimport json\n\nproducer = KafkaProducer(bootstrap_servers='kafka:9092', value_serializer=lambda v: json.dumps(v).encode('utf-8'))\n\ndef scrape_and_publish_news(url: str, topic: str):\n    response = requests.get(url, timeout=5)\n    response.raise_for_status()\n    article_data = {\"url\": url, \"content\": response.text[:500]}\n    producer.send(topic, article_data)\n    print(f\"Published {url} to Kafka topic {topic}\")",
    "img": null,
    "gallery": [],
    "tech": [
      "Apache Kafka",
    ...
    "desc": "This platform delivers real-time media intelligence by ingesting, processing, and visualizing news articles from various sources. It leverages a distributed lakehouse architecture to provide scalable data management and analytics capabilities for enterprise needs.",
    "features": [
      "Real-time data ingestion and stream processing from diverse news sources.",
      "Scalable Enterprise Lakehouse (Medallion Architecture) for structured and unstructured data.",
      "Automated ETL/ELT pipelines orchestrated by Apache Airflow for data transformation.",
      "Integrated dashboards for media intelligence visualization and infrastructure monitoring."
    ],
  ...
  },
  {
    ...
    "codeSnippet": "import { AIModelProvider } from './aiModelProvider';\n\ninterface GenerationOptions { temperature?: number; maxLength?: number; }\n\nexport async function generateFromPrompt(\n  modelId: string,\n  prompt: string,\n  options?: GenerationOptions\n): Promise<string> {\n  const model = AIModelProvider.get(modelId);\n  const result = await model.complete(prompt, options);\n  return result.text;\n}",
    "img": null,
    "gallery": [],
    "tech": [
      "React",
    ...
    "desc": "OmniPulse AI Studio delivers a sophisticated web platform designed for seamless interaction and management of AI models, enhancing developer and user productivity. It showcases a modern full-stack architecture, combining a reactive TypeScript frontend with robust backend services, deployed via containerization and CI/CD workflows.",
    "features": [
      "Component-driven frontend architecture with React and TypeScript for rich user experiences",
      "Containerized application deployment using Docker, ensuring portability and consistent environments",
      "Scalable API backend facilitating seamless integration with AI models and data processing",
      "Automated CI/CD pipelines via Vercel and GitHub Actions for rapid, reliable deployments"
    ],
